    ...
    <div class="card">
        ...
        <div class="card-body">
            <table class="table table-bordered table-striped">
                ...
                <tbody>
                    @foreach ($admin_data['categories'] as $categorie)
                        <tr>
                            <td>{{ $categorie->getId() }}</td>
                            <td>{{ $categorie->getName() }}</td>
                            <td>
                                <a class="btn btn-primary" href="{{ route('admin.categorie.edit', ['id' => $categorie->getId()]) }}">
                                    <i class="bi-pencil"></i>
                                </a>
                            </td>
                            <td>
                                <form action="{{ route('admin.categorie.delete', $categorie->getId()) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
